...
def inserir(artigo: Artigo) -> Optional[int]:
    ...
    try:
        print(f"DEBUG - Inserindo artigo: titulo={artigo.titulo}, status={artigo.status}, usuario_id={artigo.usuario_id}, categoria_id={artigo.categoria_id}")
        with get_connection() as conn:
            cursor = conn.cursor()
            ...
            print(f"DEBUG - Artigo inserido com ID: {last_id}")
            return last_id
    except Exception as e:

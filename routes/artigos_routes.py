...
async def post_cadastrar(
    ...
    try:
        logger.info(f"DEBUG - Dados recebidos: titulo={titulo}, status={status}, categoria_id={categoria_id}, usuario_id={usuario_logado['id']}")
        ...
        logger.info(f"DEBUG - DTO criado com sucesso")

        # Verifica se título já existe
        if artigo_repo.titulo_existe(dto.titulo):
            logger.warning(f"DEBUG - Título já existe: {dto.titulo}")
        ...
        logger.info(f"DEBUG - Tentando inserir artigo...")
        artigo_id = artigo_repo.inserir(novo_artigo)
        logger.info(f"DEBUG - Resultado da inserção: {artigo_id}")
